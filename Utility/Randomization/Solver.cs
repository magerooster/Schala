...
public static partial class Solver
{
    ...
    public static Dictionary<string, ParserFunction> CustomFunctions = [];
    ...
    internal static IReadOnlyDictionary<Regex, RegexEvaluator> ParsingOrder { get; } = new Dictionary<Regex, RegexEvaluator>
    {
        //{new Regex(@"@@"), GetRandomUserFromChannel },
        {MyRegex(), SetGlobalVariable },             //$varname=value$
    ...
    {
        if (Data.Global.Variables.TryGetValue(Match.Groups[1].Value, out string? value))
        {
            return value;
        }
    ...
    {
        if (!Data.User.TryGetValue(State.State.UserID, out UserMetadata? value))
        {
            value = new UserMetadata();
            Data.User.Add(State.State.UserID, value);
        }

        value.Variables[Match.Groups[1].Value] = Match.Groups[2].Value;
    ...
    public static string GetPersonalVariable(Match Match, ParserState State)
    {
        ...
        if (value.Variables.ContainsKey(Match.Groups[1].Value))
        {
            return value.Variables[Match.Groups[1].Value];
        }

        State.State.AppendToBlock("Variable '" + value.Variables[Match.Groups[1].Value] + "' not defined.\n");
        return "";
    }
    ...
    public static string RandomExpressionFromSet(Match Match, ParserState State)
    {
        ...
        return split.Length > 0 ? split[State.Randomizer.NextInt32() % split.Length] : "";
    }
    ...
    public static string Randomize(long Dice, long Sides, long Keep, long Botch, long Autos, ParserState State, Func<PRNG.GenericRandEx, string>? DiceResult)
    {
        ...
        if (TNsub.Item1 > 0 && !int.TryParse(State.Message.AsSpan(TNsub.Item1 + 2, TNsub.Item2 - 2), out TN))
            TN = int.MaxValue;
        ...
        if (Keep == Dice)
        {
            PRNG.BaseRandEx Rnd = TN == int.MaxValue
                ? new PRNG.GenericRandEx(Dice, Sides)
                : (PRNG.BaseRandEx)new PRNG.GenericWithTargetRandEx(Dice, Sides, TN, Botch, Autos, DiceRollResult: DiceResult ?? PRNG.GenericWithTargetRandEx.WoDResult);
            Rnd.Roll();
        ...
        return TN == int.MaxValue ? total.ToString() : succs.ToString();
        //  function RKH(x, y, z)
    ...
    {
        if (int.TryParse(Match.Groups[1].Value, out int Dice) && int.TryParse(Match.Groups[2].Value, out int Sides))
        {
            return int.TryParse(Match.Groups[3].Value, out int Keep)
                ? Randomize(Dice, Sides, Keep, 0, 0, State, null)
                : Randomize(Dice, Sides, Dice, 0, 0, State, null);
        }
    ...
    public static string Exponent(Match Match, ParserState State)
    {
        ...
        return !int.TryParse(Match.Groups[2].Value, out int y) ? "0" : Math.Pow(x, y).ToString();
    }
    public static string Multiply(Match Match, ParserState State)
    {
        ...
        return !int.TryParse(Match.Groups[2].Value, out int y) ? "0" : (x * y).ToString();
    }
    ...
    public static string ExecuteCustomFunction(Match Match, ParserState State)
    {
        ...
        return CustomFunctions.TryGetValue(Match.Groups[1].Value, out ParserFunction? value)
    ...
    private static readonly string TNRegex = "[tT][nN][0-9]*";
    #endregion
    ...
    public static string Parse(ParserState State, IReadOnlyDictionary<Regex, RegexEvaluator>? CustomLogic = null)
    {
        ...
        string prevWorking = "";
        Tuple<bool, string> result = working.RegexGsub(CommentRegex, "");
        //Main parsing loop
