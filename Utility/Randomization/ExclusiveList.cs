...
public class ExclusiveList<T> : IList<T> where T : IComparable<T>
{
    ...
    readonly IComparer<T>? Comparer = null;
    ...
    public int MaxSize
    {
        ...
        set
        {
            ...
            if (_list != null && field < _list.Count)
                _list.RemoveRange(field, _list.Count - field);
        }
    }
    #endregion
    #region Constructors
    ...
    {
        this.Comparer = Comparer ?? HighestComparer;
    }
    ...
    public static Comparer<T> HighestComparer = Comparer<T>.Create((x, y) => x.CompareTo(y) * -1);
    public static Comparer<T> LowestComparer = Comparer<T>.Create((x, y) => x.CompareTo(y));
    #endregion
