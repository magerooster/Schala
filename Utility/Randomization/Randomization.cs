...
{
    public class FudgeDiceRandEx : BaseRandEx
    {
        protected static CustomDie FudgeDie = new CustomDie(
            new DieFace(-1, "[-]"),
            new DieFace(-1, "[-]"),
            new DieFace(0, "[ ]"),
            new DieFace(0, "[ ]"),
        ...
        public FudgeDiceRandEx(int Dice = 1)
        {
            if (Dice > 0)
                this.Dice = Dice;
            else
    ...
    public class GenericWithTargetRandEx : GenericRandEx
    {
        ...
        public static Func<GenericRandEx, string> WoDResult = (rngState) =>
        {
            ...
            if (rngState is GenericWithTargetRandEx state)
            {
                ...
                if (state.Hits == 0 && state.Botches > 0)
                    nonSpec = "**Botch**";
                else if (state.Botches > state.Hits)
                    nonSpec = $"**Failure**";
                else
                    nonSpec = $"{state.Hits - state.Botches} Successes";

                if (state.Hits + state.MaxHits == 0 && state.Botches > 0)
                    spec = "**Botch**";
                else if (state.Botches > state.Hits + state.MaxHits)
                    spec = $"**Failure**";
                else
                    spec = $"{state.MaxHits + state.Hits - state.Botches} Successes";
    ...
    {
        #region Basic Data
        protected static SFMT Rnd = new SFMT();
        ...
        protected StringBuilder _header = new StringBuilder();
        ...
        protected StringBuilder _blockText = new StringBuilder();
        ...
        protected int GetRandLimit(long Sides)
        {
            int s = (int)Sides;
            return int.MaxValue - ((int.MaxValue % s) + 1) % s;
        }

        protected int RollDie(int Sides, int Limit)
        {
            int r = 0;
    ...
    public class CustomDie
    {
        ...
        public DieFace[]? Faces { get; set; } = null; //1 Indexed because we're dealing with dice.
        
        public CustomDie(params DieFace[] Faces)
        {
            Sides = Faces.Length;
            this.Faces = Faces;
        }
    ...
    public class DieFace
    {
        ...
        public DieFace(int Value, string Output)
        {
            this.Output = Output;
            this.Value = Value;
        }
