...
{
    [Command("hello")]
    public static async Task HelloWorldCommandAsync(CommandContext ctx)
    {
    ...
    [Command("roll")]
    public static async Task SolveCommand(CommandContext ctx, string Phrase)
    {
    ...
    [Command("rollhidden")]
    public static async Task HiddenSolveCommand(CommandContext ctx, string Phrase)
    {
        ParserState state = new(ctx, Phrase);
    ...
    [Command("as")]
    public static async Task PostAs(CommandContext Context, string Name, string Message)
    {
        CommandState State = new(Context);
        await State.StartResponseAsync(false);
        await State.FinishResponseAsync($"> {Message}  \n*-{Name}* ({State.UsernameForHeader})");
    }

    [Command("wod")]
    public static async Task OldWoDCommand(CommandContext ctx,
        long DicePool,
        long TargetNumber = 6,
        ...
        string Description = "")
    {
        ParserState state = new(ctx, $"{DicePool}d10 TN{TargetNumber} {Description}");
        await state.State.StartResponseAsync(false);
    ...
    [Command("wodhidden")]
    public static async Task HiddenOldWoDCommand(CommandContext ctx,
        long DicePool,
        long TargetNumber = 6,
        long AutoSuccesses = 0,
        bool IsCaste = false,
    ...
    [Command("rooster")]
    public static async Task Emergency(CommandContext ctx)
    {
        if (ctx.User.Id == 899508644868128789)
        {
            CommandState State = new(ctx);
...
public class SchalaGlobalCommandGroups
{
    ...
    {
        [Command("roll")]
        public static async Task HelpRollCommand(CommandContext ctx)
        {
            CommandState state = new(ctx);
            await state.StartResponseAsync(true);

            const string response = "**/roll** takes a natural paragraph and looks for specific patterns and 'solves' them. For example, you could say 'I have 3d6 bananas' and Schala's response could be 'I have 12 bananas'.\n" +
                              "**Recognized patterns:**\n" +
    ...
    {
        [Command("pa")]
        public static async Task GenesysRollProfAbilityCommand(CommandContext ctx,
        ...
        [Command("cs")]
        public static async Task GenesysRollCharSkillCommand(CommandContext ctx,
        ...
        [Command("pahidden")]
        public static async Task GenesysRollProfAbilityCommandHidden(CommandContext ctx,
            long YellowProficiencyDice = 0,
            long GreenAbilityDice = 0,
            long PurpleDifficultyDice = 0,
            long RedChallengeDice = 0,
        ...
        [Command("cshidden")]
        public static async Task GenesysRollCharSkillCommandHidden(CommandContext ctx,
            long CharacteristicValue = 0,
            long SkillValue = 0,
            long PurpleDifficultyDice = 0,
            long RedChallengeDice = 0,
        ...
        [Command("edit")]
        public static async Task EditGenesysRoll(CommandContext ctx,
        ...
        [Command("crit")]
        public static async Task GenesysCritCommand(CommandContext ctx, long modifier)
        {
            CommandState State = new(ctx);
            await State.StartResponseAsync(false);
        ...
        [Command("crithidden")]
        public static async Task GenesysCritCommandHidden(CommandContext ctx, long modifier)
        {
            CommandState State = new(ctx);
            await State.StartResponseAsync(true);
            //await State.ReplyAsync(SpecialDiceRollers.GenesysCritTable(State, modifier));
        ...
        [Command("shipcrit")]
        public static async Task StarWarsShipCritCommand(CommandContext ctx, long modifier)
        {
            CommandState State = new(ctx);
            await State.StartResponseAsync(false);
            //await State.ReplyAsync(SpecialDiceRollers.GenesysCritTable(State, modifier));
        ...
        public static async Task StarWarsShipCritCommandHidden(CommandContext ctx, long modifier)
        {
            CommandState State = new(ctx);
            //await State.ReplyAsync(SpecialDiceRollers.GenesysCritTable(State, modifier));
            await State.StartResponseAsync(true);
    ...
    {
        [Command("wipe")]
        public static async Task WhyWipe(CommandContext Context)
        {
            CommandState state = new(Context);
            await state.StartResponseAsync(false);
            Random rng = new();
        ...
        [Command("roulette")]
        public static async Task DutyFinderSilliness(CommandContext Context)
        {
            CommandState state = new(Context);
            await state.StartResponseAsync(false);
            Random rng = new();
    ...
    public class Stats
    {
        ...
        [Command("rooster")]
        public static async Task Rooster(CommandContext Context,
            long MinimumScore = 8,
            long Count = 32,
        ...
        [Command("stat")]
        public static async Task PointBuyStats(CommandContext Context, long PointBuy)
        {
            CommandState State = new(Context);
            await State.StartResponseAsync(false);
            ...
            Dictionary<int, int> pointBuyValues = new()
            {
                {8, 0},
                {9, 1},
                {10, 2},
    ...
    {
        [Command("create")]
        public static async Task CreateList(CommandContext ctx, string ListName)
        {
            CommandState state = new(ctx);
            await state.StartResponseAsync(true);
        ...
        [Command("set")]
        public static async Task SetValue(CommandContext ctx,
            string ListName,
            string Value,
            long Weight)
        {
        ...
        [Command("view")]
        public static async Task View(CommandContext ctx, string Name, long Page = 1)
        {
            CommandState state = new(ctx);


            if (SchalaClient.Current.WeightedLists.FirstOrDefault(l => l.Name == Name) is not WeightedList list)
            {
...
public partial class SchalaZealCommands
{
    #region Event Creation Strings
    private readonly Dictionary<string, string> PresetEventTypes = new()
    {
        {"leaders", "<@&176046961441374208> <@&176047023051636737> meeting"}, //@Royalty @Guru meeting
        {"group2", "static raid ex" },
    };

    private static readonly Regex DateRegex = MyRegex();
    private static readonly Regex TimeRegex = new(@"^(\d{1,2})(?>:(\d{0,2}))?([aApP][mM])?$");
    #endregion
        ...
        string Description = "*No description for event provided.*")
    {
        ...
        DiscordEmbedBuilder success = new();
        ...
        {
            if (keyword.Length == 0) //Weirdness with the string split.
                continue;

            //Check to see if it matches the name of a roll.
            string lower = keyword.ToLower();
            DiscordRole? role = guild.Roles.FirstOrDefault(r => r.Value.Name.Equals(lower, StringComparison.CurrentCultureIgnoreCase)).Value;
            if (role is not null)
            {
            ...
            if (timeResult.Success)
            {
                ...
                Minute = timeResult.Groups[2].Success ? int.Parse(timeResult.Groups[2].Value) : 0;
        ...
        DateTime finalDate = new(Year, Month, Day, Hour, Minute, 0);
        if (dayOfWeek >= 0)
        {
        ...
        const string connString = "Data Source=schala.db";
        ...
        await using (SqliteConnection db = new(connString))
        {
            db.Open();
        ...
        success.AddField("Date", $"{finalDate.Month:D2}/{finalDate.Day:D2}", true);
        success.AddField("Time", $"{finalDate.Hour:D2}:{finalDate.Minute:D2}", true);
        string finalRoles = RolesToPing.Count > 0 ? string.Join("\n", RolesToPing) : "(None)";
    ...
    [Command("report")]
    public static async Task Report(CommandContext Context, string Complaint)
    {
        CommandState state = new(Context);
        await state.StartResponseAsync(true);
    ...
    [Command("test")]
    public static async Task Test(CommandContext context, string Text)
    {
        CommandState state = new(context);
        await state.StartResponseAsync(true);
    ...
    [GeneratedRegex(@"^([mM]\d{1,2})?([dD]\d{1,2})$")]
