...
public partial class SchalaClient : ObservableBackgroundService
{
    ...
    public Dictionary<Tuple<ulong, ulong, SupportedCommandToEdit>, PreviousCommand> PreviousMessages = [];
    #endregion
    ...
    public static async Task OnMessageUpdated(MessageUpdatedEventArgs e)
    {
        await Task.Delay(0);
        return;
    }
    ...
    public static void Log(LogLevel Severity, EventId Id, string Message)
    {
        if (Current == null)
        {
            Debug.WriteLine($"[{Severity}] [{Id.Name}] {Message}");
        }
        else
        {
    ...
    public static async Task Run(string ActionName, Action action, TimeSpan period, CancellationToken cancelToken)
    {
    ...
    public void LoadEmojis(DiscordGuild Guild)
    {
        ...
        Dictionary<string, string> guildEmojis = [];
        if (Guild.Name != null && !GuildEmojiLookup.ContainsKey(Guild.Id))
        {
            ...
            foreach (var emoji in Guild.Emojis)
            {
                if (emoji.Value?.Name != null && !guildEmojis.ContainsKey(emoji.Value.Name))
                {
    ...
    {
        return GuildEmojiLookup.ContainsKey(guildID)
            ? GuildEmojiLookup[guildID].TryGetValue(emojiName, out string? value)
                ? value : $"[Guild {guildID} does not have an emoji named {emojiName}]"
            : $"[Not currently connected to {guildID}]";
    }
    ...
    public static Task StartClock(DiscordGuild Guild) //This should only be called for Zeal.
    {
        return Task.Delay(0);
